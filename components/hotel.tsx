        ...
        {rooms.map((room, roomIndex) => (
          <div key={room.id} className="mb-6 p-6 bg-white shadow-md rounded-lg">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-2xl font-semibold text-gray-800">{room.type.charAt(0).toUpperCase() + room.type.slice(1)} Room ({room.occupants.length} person{room.occupants.length > 1 ? 's' : ''})</h2>
            ...
            {room.occupants.map((occupant, occupantIndex) => (
              <div key={occupantIndex} className="mb-4">
                <Label htmlFor={`occupant-${room.id}-${occupantIndex}`} className="text-sm font-medium text-gray-700">
                  Person {occupantIndex + 1}
                </Label>
                <Input
                  id={`occupant-${room.id}-${occupantIndex}`}
                  value={occupant}
